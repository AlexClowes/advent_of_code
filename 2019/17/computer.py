...
def int_code_computer(program, input_queue, output_queue):
    program = list(program) + [0] * 2 ** 20
    pos = 0
    relative_base = 0
    while True:
        ...
        if op_code in [1, 2, 7, 8]:
            ...
            program[param] = input_queue.get()
            if program[param] == StopIteration:
                break
            pos += 2
        elif op_code == 4:
            # Output
...
def launch_int_code_computer(program):
    in_queue = Queue()
    out_queue = Queue()
    Thread(target=int_code_computer, args=(program, in_queue, out_queue)).start()
